...
.controller('homeCtrl', function($scope, $state, $cordovaGeolocation,$ionicLoading,HomeService,Eventful,$document,$http) {  
...
map = new google.maps.Map(document.getElementById("map"), mapOptions);
    ...
    $http.get('data.json').success(function (data) {
       console.log("this is my start data rashmi"+data.length);   
       var count;
       for(count=0;count<data.length;count++)
        {
            console.log(data[count].LATITUDE);
             map=$scope.map;
             var concert_center = new google.maps.LatLng(data[count].LATITUDE, data[count].LONGITUDE);
